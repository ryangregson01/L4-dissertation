...
Finally, all of these classifiers perform better than a dummy baseline. This can be seen in Figure \ref{fig:roctest} because at a TPR of 0.5 there is a FPR less than 0.5; with the worst classifier only recording approximately a 0.2 FPR at a 0.5 TPR.
...
\begin{table}[!htb]
    ...
    \toprule
    Model architecture & Feature input & Accuracy & Precision & Recall & Specificity & F1-score & MCC \\ \midrule
    \multirow{2}{*}{CNN (2D)} & One-hot & 0.6351 & 0.1766 & 0.5551 & 0.6466 & 0.2256 & 0.1195 \\
     & PSSM & 0.4461 & 0.156 & 0.7758 & 0.3967 & 0.2318 & 0.0982 \\
    \multirow{2}{*}{CNN (1D)} & One-hot & \textbf{0.7401} & 0.2148 & 0.4367 & \textbf{0.7647} & 0.2458 & 0.1587 \\
     & PSSM & 0.7045 & 0.2279 & 0.5819 & 0.7096 & 0.2895 & \textbf{0.1909} \\
    \multirow{2}{*}{RNN (LSTM)} & One-hot & 0.5414 & 0.1959 & \textbf{0.7891} & 0.4869 & 0.2775 & 0.1636 \\
     & PSSM & 0.6596 & \textbf{0.2337} & 0.6108 & 0.6479 & \textbf{0.2896} & 0.1809 \\ \cmidrule(l){1-8} 
    \end{tabular}
...
\begin{figure}[!htb] 
    ...
    \end{subfigure}
    \caption{Confusion matrices for CNN with 1D convolution kernels within its architecture. \subref{fig:cf2d1hot} shows the confusion matrix for the model using one-hot encoding feature input. \subref{fig:cf2dpssm} shows the confusion matrix for the model using PSSM feature input. We see this models architecture correctly identifies only half of the disordered amino acids. There is a minimal difference between the confusion regarding different inputs, and both of these models have a similar overall confusion as the CNN with 2D convolutions and one-hot encoding input (Figure \ref{fig:caspcf2d1hot}).}
    \label{fig:caspcf1d}
\end{figure}
...
\begin{figure}[!htb] 
    ...
    \end{subfigure}
    \caption{Confusion matrices for RNN using an LSTM architecture. \subref{fig:cf2d1hot} shows the confusion matrix for the model using one-hot encoding feature input. \subref{fig:cf2dpssm} shows the confusion matrix for the model using PSSM feature input. We see this models architecture experiencing the least confusion again.}
    \label{fig:caspcfrnn}
\end{figure}
...
\begin{figure}[!htb]
    ...
    \caption{ROC curves of probability-based IDR predictions for our 6 models on the CASP10 dataset. The CNN (1D) and LSTM models using one-hot encoding have the best performance given a TPR and FPR trade off. Both CNN (2D) models perform the poorest. Furthermore, every model does perform above the dummy baseline.}
...
\begin{table}[!htb]
    ...
    Model architecture & \multicolumn{2}{c}{CNN (2D)} & \multicolumn{2}{c}{CNN (1D)} & \multicolumn{2}{c}{RNN (LSTM)} \\ \cmidrule{0-0}
    Feature input & One-hot & PSSM & One-hot & PSSM & One-hot & PSSM \\ \midrule
    AUC & 0.5958 & 0.6196 & \textbf{0.7511} & 0.7115 & 0.7477 & 0.7021 \\ \bottomrule
    \end{tabular}
    
    \label{tab:auccasp}
\end{table}

When analysing ROC curves with our CASP10 dataset, we see that the CNN (1D) and LSTM using one-hot encoding features achieve the best AUC (Table \ref{tab:auccasp}). This is interesting because it is a different conclusion compared to the other results. We notice when considering the CNN (1D) and LSTM architectures that the one-hot encoding ROC curves are similar and the PSSM ROC curves are similar. This implies that different feature encodings may represent protein sequences in a fashion that different architectures can easily pickup similar feature patterns, regardless of the underlying architecture. We again conclude that the CNN with 2D convolutions in its architecture has the worst performance on this dataset. Furthermore, we can conclude that all of these models again perform better than the dummy baseline from inspection of that ROC curve graph (Figure \ref{fig:roccasp}), demonstrates at a 0.5 TPR, the FPR is below 0.5 for all models.

\subsection{Comparison of other solutions}
[Table comparison]

\section{Evaluation of the web server}
[Screenshot of prediction made by this web server and a different web server. Similarities/differences.]
